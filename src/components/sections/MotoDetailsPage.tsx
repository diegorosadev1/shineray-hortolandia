...
export const MotoDetailsPage: React.FC<{ openModal?: () => void }> = ({ openModal }) => {
  const { id } = useParams<{ id: string }>();
  const [activeTab, setActiveTab] = useState<'interested' | 'details' | 'finance' | 'trade'>('interested');
  ...
  return (
    <div className="bg-[#f5f5f5] min-h-screen pb-20">
      {/* Breadcrumbs */}
      <div className="max-w-screen-2xl mx-auto px-4 py-4 hidden md:flex items-center gap-2 text-[10px] font-bold uppercase italic tracking-widest text-gray-400">
        <Link to="/" className="hover:text-[#ff0000] transition-colors">Início</Link>
        <ChevronRight size={10} />
        <Link to="/estoque" className="hover:text-[#ff0000] transition-colors">NOSSO ESTOQUE</Link>
        <ChevronRight size={10} />
        <span className="text-black">{moto.name}</span>
      </div>
      ...
      <div className="max-w-screen-2xl mx-auto px-4">
        ...
        <div className="flex flex-col lg:flex-row mb-8">
          <div className="flex-1 bg-black p-8 pt-16 lg:p-12 lg:pt-24 relative overflow-hidden">
            <div className="relative z-10">
              <h1 className="text-white text-3xl md:text-5xl font-black italic uppercase leading-none mb-4 max-w-2xl">
        ...
        <div className="mt-12">
          ...
          <div className="bg-white border border-gray-100 min-h-[400px]">
            {activeTab === 'interested' && <InterestedTab moto={moto} openModal={openModal} />}
        ...
        <div className="mt-20">
          ...
          <div className="flex md:grid md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8 overflow-x-auto md:overflow-x-visible pb-8 md:pb-0 snap-x snap-mandatory scrollbar-hide -mx-4 px-4 md:mx-0 md:px-0">
            {MOCK_MOTOS.filter(m => m.id !== id).slice(0, 3).map(m => (
              <div key={m.id} className="min-w-[280px] sm:min-w-[320px] md:min-w-0 snap-center">
                <MotoCard moto={m} />
              </div>
            ))}
          </div>
...
const InterestedTab = ({ moto, openModal }: { moto: any, openModal?: () => void }) => (
  <div className="relative overflow-hidden">
    <div className="p-4 md:p-12 grid grid-cols-1 lg:grid-cols-2 gap-8 md:gap-12">
      ...
      <div className="bg-[#1A1A1A] p-6 md:p-12 rounded-[2.5rem] md:rounded-[32px] text-white relative overflow-hidden">
        <div className="relative z-10">
          <span className="text-[#ff0000] font-black tracking-[0.3em] uppercase text-[10px] mb-4 block italic">Interesse</span>
      ...
      <div className="flex flex-col justify-center items-center text-center p-4 md:p-8">
        <div className="max-w-sm">
          <h3 className="text-2xl md:text-3xl font-black text-[#1A1A1A] uppercase italic tracking-tighter mb-4 leading-tight">
            Prefere que a gente <br className="hidden md:block" /> entre em contato?
          </h3>
          ...
          <div className="mt-8 md:mt-12 pt-8 md:pt-12 border-t border-gray-100 flex flex-col items-center gap-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Consultores Online</p>
            </div>
          </div>
        </div>
      </div>
...
const DetailsTab = () => (
  <div className="p-6 md:p-12">
    <h3 className="text-2xl md:text-3xl font-black italic uppercase text-[#ff0000] mb-8">Diferenciais</h3>
    <div className="text-[10px] font-black uppercase tracking-[0.2em] text-gray-400 mb-8">Acessórios Inclusos:</div>
    
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
      {[
        'Partida Elétrica', 'Freio a Disco', 'Injeção Eletrônica', 
        'Painel Digital', 'ABS', 'Controle de Tração', 
        'Protetor de Motor', 'Bauleto', 'Manual do Proprietário',
        'Chave Reserva', 'Pneus Novos', 'Revisada'
      ].map((item, idx) => (
        <div key={idx} className="flex items-center gap-3 group">
          <CheckCircle2 size={18} className="text-[#ff0000] group-hover:scale-110 transition-transform" />
          <span className="text-xs font-bold uppercase text-gray-700">{item}</span>
        </div>
      ))}
    </div>
  </div>
        ...
        <div className="flex flex-col gap-6 md:gap-8">
          ...
          <div className="flex flex-col gap-3 md:gap-4 px-2 md:px-0">
            <div className="bg-white border border-gray-100 p-4 md:p-5 rounded-2xl shadow-sm flex items-center gap-4 group hover:shadow-md transition-all">
              ...
              <span className="text-black font-black italic uppercase text-[10px] md:text-xs tracking-tight">Aprovação em 30min</span>
            </div>
            <div className="bg-white border border-gray-100 p-4 md:p-5 rounded-2xl shadow-sm flex items-center gap-4 group hover:shadow-md transition-all">
              <CheckCircle2 className="text-[#ff0000]" size={20} />
              <span className="text-black font-black italic uppercase text-[10px] md:text-xs tracking-tight">Zero de Entrada</span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-[2.5rem] md:rounded-[40px] p-6 md:p-10 shadow-[0_20px_60px_-15px_rgba(0,0,0,0.1)] border border-gray-50 flex flex-col gap-8 md:gap-10">
          ...
          <div className="flex flex-col gap-10 md:gap-12">
            <div className="flex flex-col gap-6">
              <label className="text-[#444] text-[10px] font-black uppercase tracking-widest">Valor de Entrada</label>
            ...
            <div className="bg-gray-50 p-4 rounded-xl flex justify-between items-center">
              <span className="text-gray-500 text-[10px] font-black uppercase tracking-widest">Valor a Financiar:</span>
              <span className="text-black font-black italic text-base md:text-lg">{formatCurrency(financingAmount)}</span>
            </div>
          </div>
          ...
          <form className="flex flex-col gap-5 md:gap-6" onSubmit={(e) => e.preventDefault()}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 md:gap-6">
              <div className="flex flex-col gap-2">
                <label className="text-[#444] text-[10px] font-black uppercase tracking-widest ml-1">Telefone</label>
                <input 
                  type="text" 
                  placeholder="(00) 00000-0000"
                  className="w-full bg-gray-50 border-none rounded-xl p-4 text-sm focus:ring-2 focus:ring-[#ff0000] outline-none transition-all"
...
const TradeTab = ({ moto }: { moto: any }) => (
  <div className="p-8 lg:p-12">
    <h3 className="text-3xl font-black italic uppercase text-[#ff0000] mb-4">Quero Trocar</h3>
    <p className="text-gray-500 text-sm font-medium mb-12">Tem uma moto e quer dar como entrada? Preencha os dados abaixo para uma pré-avaliação.</p>

    <form className="grid grid-cols-1 md:grid-cols-2 gap-6">
      <div className="space-y-4">
        <div className="text-[10px] font-black uppercase tracking-widest text-gray-400">Dados da sua Moto</div>
        <input type="text" placeholder="Marca / Modelo" className="w-full bg-gray-50 border border-gray-100 p-4 text-xs font-bold uppercase outline-none focus:border-[#ff0000]" />
        <input type="text" placeholder="Ano" className="w-full bg-gray-50 border border-gray-100 p-4 text-xs font-bold uppercase outline-none focus:border-[#ff0000]" />
        <input type="text" placeholder="KM Atual" className="w-full bg-gray-50 border border-gray-100 p-4 text-xs font-bold uppercase outline-none focus:border-[#ff0000]" />
      </div>
      <div className="space-y-4">
        <div className="text-[10px] font-black uppercase tracking-widest text-gray-400">Seus Dados</div>
        <input type="text" placeholder="Seu Nome" className="w-full bg-gray-50 border border-gray-100 p-4 text-xs font-bold uppercase outline-none focus:border-[#ff0000]" />
        <input type="tel" placeholder="Telefone" className="w-full bg-gray-50 border border-gray-100 p-4 text-xs font-bold uppercase outline-none focus:border-[#ff0000]" />
